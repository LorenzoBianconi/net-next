...
static u32 mtk_hw_reset_check_fe(struct mtk_eth *eth)
{
	u32 val;
...
static void mtk_hw_warm_reset(struct mtk_eth *eth)
{
	...
	regmap_update_bits(eth->ethsys, ETHSYS_RSTCTRL, RSTCTRL_FE,
			   RSTCTRL_FE);
	if (readx_poll_timeout(mtk_hw_reset_check_fe, eth, val,
	...
	regmap_read(eth->ethsys, ETHSYS_RSTCTRL, &val);
	if (!(val & rst_mask))
		dev_err(eth->dev, "warm reset s0 fail %08x (%08x)\n", val,
			rst_mask);
	...
	regmap_read(eth->ethsys, ETHSYS_RSTCTRL, &val);
	if (val & rst_mask)
		dev_err(eth->dev, "warm reset s1 fail %08x (%08x)\n", val,
			rst_mask);
}
