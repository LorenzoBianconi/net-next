...
#define AIROHA_NUM_HTB_QUEUES		8
#define AIROHA_NUM_NETDEV_TX_RINGS	(AIROHA_NUM_TX_RING + \
					 AIROHA_NUM_HTB_QUEUES)
...
struct airoha_gdm_port {
	...
	DECLARE_BITMAP(qos_sq_bmap, AIROHA_NUM_HTB_QUEUES);
	/* qos stats counters */
	u64 cpu_tx_packets;
	u64 fwd_tx_packets;
};
...
static void airoha_qdma_init_qos_stats(struct airoha_qdma *qdma)
{
	...
	for (i = 0; i < AIROHA_NUM_HTB_QUEUES; i++) {
					  ...
					  struct tc_htb_qopt_offload *opt)
{
	...
	u32 channel = TC_H_MIN(opt->classid);
	...
	if (channel < 1 || channel > AIROHA_NUM_HTB_QUEUES) {
		NL_SET_ERR_MSG_MOD(opt->extack, "invalid qos channel id");
		return -EINVAL;
	}
	...
	qid = channel - 1;
	set_bit(qid, port->qos_sq_bmap);
	opt->qid = AIROHA_NUM_TX_RING + qid;
					   ...
					   struct tc_htb_qopt_offload *opt)
{
	...
	if (!test_bit(qid, port->qos_sq_bmap)) {
		NL_SET_ERR_MSG_MOD(opt->extack, "invalid queue id");
		return -EINVAL;
	}

	airoha_tc_remove_htb_queue(port, qid);
...
static int airoha_tc_htb_destroy(struct airoha_gdm_port *port)
{
	...
	for_each_set_bit(q, port->qos_sq_bmap, AIROHA_NUM_HTB_QUEUES)
