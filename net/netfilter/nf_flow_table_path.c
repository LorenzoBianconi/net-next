	...
	for (i = 0; i < stack->num_paths; i++) {
		...
		case DEV_PATH_TUN:
			...
			if (path->type == DEV_PATH_TUN) {
				...
				info->tun.l3_inner_proto = path->tun.l3_inner_proto;
				info->num_tuns++;
			} else {
					...
					enum ip_conntrack_dir dir)
{
	...
	nf_route(nft_net(pkt), &tun_dst, &fl, false, tun->l3_inner_proto);
	if (!tun_dst)
		return -ENOENT;
				...
				struct nft_flowtable *ft)
{
	...
	if (info.num_tuns &&
		...
		route->tuple[!dir].in.tun.l3_inner_proto = info.tun.l3_inner_proto;
