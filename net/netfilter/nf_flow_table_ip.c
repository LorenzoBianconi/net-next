				...
				struct flow_offload_tuple *tuple)
{
	...
	if (ctx->tun.proto == IPPROTO_IPIP || ctx->tun.proto == IPPROTO_IPV6) {
		...
		case htons(ETH_P_IP):
			...
			tuple->tun.l3_inner_proto = AF_INET;
			break;
		case htons(ETH_P_IPV6):
			...
			tuple->tun.l3_proto = ctx->tun.proto;
			tuple->tun.l3_inner_proto = AF_INET6;
			break;
		default:
			break;
		}
			...
			const struct nf_hook_state *state)
{
	...
	case FLOW_OFFLOAD_XMIT_NEIGH: {
		...
		if (other_tuple->tun.l3_inner_proto ?
		    other_tuple->tun.l3_inner_proto == AF_INET6 :
		    ctx.tun.proto == IPPROTO_IPV6) {
			struct rt6_info *rt6;
			  ...
			  const struct nf_hook_state *state)
{
	...
	case FLOW_OFFLOAD_XMIT_NEIGH: {
		...
		if (other_tuple->tun.l3_inner_proto ?
		    other_tuple->tun.l3_inner_proto == AF_INET :
